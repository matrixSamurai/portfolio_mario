...
const MarioGame = () => {
  ...
  const MOVE_SPEED = 0.42; // Character movement speed (pixels per millisecond for smooth movement)
  // Snappy acceleration/deceleration for a professional, responsive feel
  const ACCELERATION = 0.008; // Acceleration rate for smooth start (higher = faster acceleration)
  const DECELERATION = 0.020; // Deceleration rate for quick stop (higher = faster stop)
  ...
  useEffect(() => {
    ...
    if (isMoving) {
      ...
      }, 110); // Change sprite every 110ms for snappy walking animation
        ...
        >
          ...
          <p className="mario-subtitle">
            {activeContent ? (
              ...
              '🎮 Jump into boxes to reveal portfolio'
            )}
          </p>
        </div>
      </div>
